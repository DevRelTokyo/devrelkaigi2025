  ...
  const getProposals = async () => {
    ...
    proposals.forEach((proposal) => {
      const userProfiles = profiles.filter(
        (p) => p.get("user").id === proposal.get("user").id
      );
      ...
      const userProfile =
        userProfiles.find((p) => p.get("lang") === proposal.get("lang")) ||
        userProfiles[0];
            ...
            <table className="table table-striped">
              ...
              <tbody>
                {proposals.map((proposal, index) => (
                  <>
                    <tr key={index}>
                      <td>{t(proposal.get("lang"))}</td>
                      <td>{proposal.get("category")}</td>
                      <td>{proposal.get("title")}</td>
                      <td>{proposal.get("profile")?.get("name")}</td>
                      <td>
                      ...
                      <td>{proposal.get("rating").toFixed(2)}</td>
                      <td>
                        {typeof proposal.get("responseStatus") !==
                        "undefined" ? (
                          ...
                          <>
                            {proposal.get("deadline") < new Date() ? (
