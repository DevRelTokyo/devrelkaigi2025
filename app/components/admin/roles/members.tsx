...
export default function AdminRolesMembers() {
  ...
  const getRole = async () => {
    ...
    setRole(role);
    const userIds = await Parse.Cloud.run('getUserIds', { id: params.id });
    setUserIds(userIds);
  };
  ...
  const addMember = async (profile: Parse.Object) => {
    ...
    await role!.save();
    setUserIds([...userIds, profile.get('user').id]);
    showMessage({
  ...
  const deleteMember = async (profile: Parse.Object) => {
    ...
    try {
      ...
      await role!.save();
      setUserIds(userIds.filter(id => id !== profile.get('user').id));
      showMessage({
                ...
                <tbody>
                  {profiles.map((profile, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        {userIds.includes(profile.get('user').id) ?
