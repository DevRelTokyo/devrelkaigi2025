  ...
  const validate = async () => {
    ...
    for (const field of schema) {
      if (field.required && !data!.get(field.name)) {
        errors.push(
          t("__label__ is required").replace("__label__", field.label)
        );
      }
      if (field.type === "array") {
        ...
        values.filter((v) => v === "").length > 0 &&
          errors.push(
            t("__sub_label__ of __label__ is required")
      ...
      if (field.pattern && value) {
        ...
        if (!re.test(data!.get(field.name))) {
          errors.push(
            t("__label__ is invalid").replace("__label__", field.label)
          );
        }
      }
                  ...
                  return (
                    <File
                      ...
                      onChange={(value: Parse.File) => {
                        ...
                        setObj({
                          ...obj,
                          [field.name]: value as unknown as FieldValue,
                        });
                      }}
                    />
                  );
