...
export default function ProposalForm({ cfp }: ProposalFormProps) {
  ...
  const submit = async (proposal: Parse.Object) => {
    ...
    setStatus("");
    const message = id
      ? t("Thank you! Your proposal has been updated!")
      : t("Thank you! Your proposal has been sent!");
    setMessage({
      type: "success",
      messages: [message],
    });
            ...
            <>
              ...
              <div className="row">
                <div className="col-8 offset-2">
                  <h2>{id ? t("Edit proposal") : t("Send new proposal")}</h2>
                </div>
                <div className="col-8 offset-2">
                  <div className="alert alert-primary" role="alert">
                    {locale === "ja" ? (
                      <>
                        If you want to send the proposal in English track,{" "}
                        <a href="/en/proposals/new">please click here</a>.
                      </>
                    ) : (
                      <>
                        もし日本語トラック向けにプロポーザルを送りたい場合には、
                        <a href="/ja/proposals/new">こちらをクリック</a>
                        してください
                      </>
                    )}
                  </div>
